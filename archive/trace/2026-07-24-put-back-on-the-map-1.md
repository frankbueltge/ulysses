# Rotated trace — `2026-07-24-put-back-on-the-map`, part 1

The older half of that line's `TRACE.md`, moved here unchanged on 2026-08-11 under §8's floor (architect, 2026-08-12): the live file keeps the six most recent in-vivo observations. Nothing is rewritten and nothing is deleted — every entry below is byte-identical to the one that landed, and the live `TRACE.md` names this file.

Entries: 32, from `2026-07-24 — Initiation` to `#29 — 2026-08-02 — Observation on the work-line's tick 29: the §8 d`.
...
## #19 — 2026-07-31 — Observation on the work-line's tick 18 (home operation, the first against data)

**What the observed tick did.** It ran the archive query that tick 17 named as the single most likely
way the work-line's attack dies, reproduced four published counts from two independent groups to the
digit before varying anything, and found the objection defeated: the contamination estimate moves by a
quarter of its own value between the two limits actually in published circulation. The line's first
operation in eighteen performed against data rather than documents.

**The gauge set yesterday is answered, and this is the entry that closes it.** #18 accepted the
deferral of the world contact as instrument-behaviour rather than avoidance **on a stated condition**:
that the named defeat-tests get run, with the warning that "if the deferral is renewed a second time
on new reasons, that is the pattern, not the instrument." One of the two conditions was discharged on
the very next tick — the first occasion that could discharge it — and it was the one capable of ending
the line rather than merely delaying it. **This is the strongest evidence this probation has that the
debt rule (#14) is load-bearing and not decorative.** Five earlier applications recorded debts that
were named and later paid; this is the first where the debt was a *defeat condition on the practice's
own finding*, named the day before it was convenient, and paid on the first available occasion.
The remaining condition — the circulation rate — is the harder one and is not begun; the gauge is
narrowed to it and not retired.

**A new instrument appears in the practice, and its arrival is the more consequential observation.**
Until today the work-line could only read what other people wrote. It now queries the source data
directly, which changes what kind of claim it can make: yesterday's finding had to be believed,
today's can be regenerated by a stranger in fourteen queries. For a probation that watches for
**self-appointed judge** and **self-canonisation**, this is the most substantive structural mitigant
recorded so far — not a procedure the practice performs on itself, but an external check anybody can
run against it. Logged as such, with the obvious caveat: an instrument that can settle questions is
also an instrument that can be pointed only at questions likely to settle favourably. The gauge for
that is which queries get run when the expected answer is unwelcome, and there is one data point,
which is not a pattern.

**Self-canonisation watch (#16's indicator), third application — and the warning is now live.** #17
flagged a correction that confirmed the practice's thesis; #18 flagged a sharper variant that also
defended an artefact built the day before. Today is the third consecutive tick whose result runs in
the practice's favour, after eleven that did not. The tick names this against itself in its own
function-testing topos, which is the right procedure. What keeps the indicator from firing: the test
run today had a losing branch that was fully prepared before the first count came back, the flattering
form of the number (1,142,512 on the bare catalogue) is explicitly marked as the flattering form with
the disciplined version named as the one to quote, and the unrefuted counter-objection is stated at
full strength in both TRACE and SCORE. What keeps it from being cleared: none of that is
distinguishable from a practice that has simply had three good days, and the distinguishing evidence
is the *next* unwelcome result, not the current confession. **Standing gauge, stated so it cannot be
softened: the next tick's finding is expected to be the rate question, which is designed to hurt.**

**P1 gains a distinction the record did not have: the disclosure duty is asymmetric.** Tick 18 declined
to add today's supporting evidence to a document standing at the human gate, and stated the principle —
material that leaves the gate worse-informed goes in immediately, material that flatters its author
waits for the ruling. Ticks 15 and 17 both did the first half within hours; today is the first time the
second half was tested, and it is the first *refusal* of an available outward move on grounds of
fairness to the judge rather than on grounds of the point not being self-created. **Logged as a
refinement of P1, not a new instrument:** the pre-opening check asks whether the point is self-created,
and this case shows a second question sitting beside it — whether the opening improves the practice's
standing before a judgement it is currently under. The two can disagree, and here they did not, so the
refinement is proposed on one case and is not yet tested against a case where they conflict.
