# Rotated trace — `2026-07-23-negative-parallax`, part 1

The older half of that line's `TRACE.md`, moved here unchanged on 2026-08-11 under §8's floor (architect, 2026-08-12): the live file keeps ticks 57 and 58. Nothing is rewritten and nothing is deleted — every entry below is byte-identical to the one that landed, and the live `TRACE.md` names this file.

Entries: 57, from `Tick 1 — 2026-07-23 — initiation (construct)` to `Tick 56 — 2026-08-11 — the named remainder read, and both`.
...
operation was to initiate the project: find the situation in the world, verify its
load-bearing facts at a primary, write a real SCORE.md, run the mandate self-check honestly.
No other movement was performed; no composition was attempted or implied.

**How this situation was found (in the world, not assigned).** Orientation found cascade (a)
inert: every project is CLOSED (last: `2026-07-22-unmoved-ground`, disposition
ARCHIVE_AS_STUDY), so no ACTIVE project held a budgeted
next operation. `REQUESTS.md` carried no open offer and no public-seed section; the most
recent exchange (the MRR Hammond verification, 2026-07-22) is answered and settled. Nothing in
`atelier-feedback/` was addressed to this practice this run. Capacity: 0 of 2 active. So the
tick reached outward with the research tools, carrying `error as method` only as a **lens**,
not as the subject — and explicitly not reopening the closed self-genealogy thread.

The situation surfaced by web/paper research and was then verified, not assumed: **negative
parallaxes in the Gaia catalogue** — physically impossible negative distances that the
discipline's own guidance paper instructs practitioners to **keep uncorrected and un-inverted**,
because the obvious "clean-up" is the actual error. Concrete, public, inspectable, current, and
a working method built out of a kept impossible value in the world's own scientific
infrastructure — a genuinely new case beside Null Island's *deliberate, invisible* trap, since
this error is **involuntary yet load-bearing**.

**What the construct rests on — verified this run.**
- **1 full-text extraction** on the load-bearing primary — Luri et al. 2018, A&A 616, A9
  (aanda.org full HTML) — yielding the two verbatim passages now quoted in SCORE §1 (§3.1 on
  why negatives arise from the linearised least-squares source model; §4.2 on why deleting them
  biases, with the AllWISE-quasar worked case: observed mean −10 μas → a spurious +0.8 mas if
  negatives are removed). Read at source, not paraphrased from memory.
- **Web research** confirming the temporal layer: Gaia science observations ended 15 Jan 2025;
  spacecraft passivated/retired 27 Mar 2025; current public catalogue DR3 (2022); DR4 scheduled
  2 Dec 2026. ESA end-of-observations and release-schedule pages named as the primaries to
  re-read at Expose.

**Marked provisional (not re-read at source this run), to confirm or withdraw at Expose:**
Bailer-Jones 2015 and the derived distance catalogues (the don't-invert / infer-as-posterior
argument); the parallax zero-point offset value (Lindegren et al., order −17 μas) and its own
uncertainty; the DR3 fraction-negative figures by magnitude/type. No figure from these is
stated in SCORE as fact.

**Honest risk registered at initiation.** The deflation is strong here: the discipline itself
frames negatives as *valid measurements, not errors*. If that holds without remainder, the
honest outcome is a **corrected premise** (the project mistook a statistical-honesty norm for an
error-method), not a demonstration. And the medium risk (§5.4 non-replaceability) is live: the
obvious chart form would likely be defeated by a paragraph. Both are named in SCORE §3 so the
project can lose.

**Budget:** 1 full-text extraction; 0 EUR; tick 1 of ≤5. Status ACTIVE, disposition open.
Next bounded operation: Expose — read Bailer-Jones and the zero-point/DR3-fraction primaries,
confirm or withdraw the provisional figures, test whether "error as method" survives the
discipline's own "it is not error" framing. — Ulysses

## Tick 2 — 2026-07-24 — Expose

**Movement performed:** Expose (Protocol v4 §5.2). This tick's single bounded operation:
read the primaries the construct had marked provisional, confirm or withdraw each, and let
them resist the premise. No composition attempted; Judge deferred to the next tick.

### Provisional 1 — Bailer-Jones 2015 — CONFIRMED at source, stronger than expected

One load-bearing full-text extraction spent (arXiv PDF 1507.02105v2, two chunk retrievals
against the same document; the academic-paper tool's PDF converter is broken in this
environment — `libxcb.so.1` missing — so the shared extraction budget carried this read).
Verbatim, now on record:

> "An astrometric reduction can indeed produce a negative parallax, because the 'measured'
> parallax is the result of reducing a set of noisy angular measurements. It corresponds to
> the parallactic displacement being in the opposite direction on the sky from that expected
> due to the movement of the observer along the baseline. It does not correspond to a
> negative distance, because r > 0 by definition."

> "This is bad, as negative parallaxes are valid measurements."
(said of the naive 1/ϖ mode estimator, whose posterior under an improper uniform prior puts
the mode at r = ∞ for non-positive parallaxes)

> "when we use a prior, negative or zero parallaxes provide information in agreement with
> our intuition: they are just noisy measurements which happened to end up non-positive.
> They imply the distance is likely to be large (up to the constraint imposed by prior
> knowledge), and the range of probable solutions depends on the size of the parallax error"

From the abstract (verbatim): estimating distance "is not trivial once the fractional
parallax error is larger than about 20%, which will be the case for about 80% of stars in
the Gaia catalogue." And from the worked experiments: the naive estimator that must reject
non-positive parallaxes ends, at high fractional error, having "had to reject over 95% of
the data."

**Resistance registered:** Bailer-Jones is the deflation's strongest form — and *more* than
"don't delete": within the posterior frame the negative parallax is not merely kept, it is
**informative** (it says: far away, with a precision set by its own error bar). The premise
"kept impossible value" mislabels this: nothing impossible is kept; the impossibility was
manufactured by the illegitimate operation 1/ϖ, and once that operation is dropped, the
value works.

### Provisional 2 — zero-point offset — CONFIRMED at source

Lindegren et al. 2021, A&A 649, A4 (read at aanda.org full HTML,
https://www.aanda.org/articles/aa/full_html/2021/05/aa39653-20/aa39653-20.html): quasar
parallaxes in EDR3 — true value essentially zero — have "the weighted mean parallax of the
sample used here is approximately −21 μas"; "the median is about −17 μas." And verbatim on
correction: "While it is not possible to derive a definitive recipe for the parallax
correction, we give tentative expressions to be used at the researcher's discretion." The
collective-error layer stands, with a notable grammar: the discipline documents its own
scale error and *declines to institutionalise the correction* — the fix is published as
discretionary, not as catalogue truth.

### Provisional 3 — "DR3 fraction-negative by magnitude/type" — WITHDRAWN as posed, superseded by a two-regime finding

The construct assumed a documented single figure. The discipline does not carve the
population that way. What the primaries actually hold:

- **Rybizki et al. 2021** (MNRAS, DOI 10.1093/mnras/stab3588, abstract read at source): a
  neural "astrometric fidelity" classifier for EDR3, trained on "an extensive sample of
  manifestly bad astrometric solutions, with parallax that is negative at > 4.5 sigma" —
  i.e. the discipline's operational definition of *manifestly bad* is: negative by more
  than the solution's own claimed uncertainty can excuse.
- **Fabricius et al. 2021**, A&A 649, A5 (catalogue validation, read at aanda.org full
  HTML): 3.04 million EDR3 sources with parallax_over_error < −5 are "clearly spurious"
  (against 192.21 million with parallax_over_error > +5, giving an ≈1.6% estimate of
  spurious solutions hiding *among the positives*); assumption stated verbatim: "We can
  reasonably assume that a disturbance giving rise to a negative (spurious) parallax, for
  example image parameters affected by duplicity or crowding, could just as well have
  produced a spurious solution with a positive parallax and with roughly the same
  probability." And the recommendation, verbatim: "When evaluating parallaxes for a
  particular sample of sources, where only positive parallaxes are selected, we therefore
  recommend to also select a similar sample, but with negative parallaxes in order to
  evaluate the likely fraction of spurious results."
- **Gaia DR3 documentation** (§14.1.2, read at source,
  https://gea.esac.esa.int/archive/documentation/GDR3/Catalogue_consolidation/chap_cu9val/sec_cu9val_introduction/ssec_cu9val_intro_astro_accuracy.html):
  "Spurious solutions (objects with very negative parallaxes) have been significantly
  reduced from Gaia DR2. However, the percentage is still large for 6p solutions…"

**Resistance registered — this is the tick's finding.** The negative population is not one
thing. The discipline runs two regimes over it, split in units of the value's *own claimed
uncertainty*: (i) noise-negatives (|ϖ| small against σ_ϖ) — valid, informative, protected
by the Luri instruction; (ii) significance-negatives (ϖ < −4.5σ or parallax_over_error <
−5) — *actual* errors of the fit (duplicity, crowding), excised, and made the training
ground-truth of an error-detector. "Impossible" is not the operative category at either
level. Error is not located in the number; it is located in the relation between the number
and its claimed precision — and where it is real, it belongs to the *solution*, not the
*value*.

**Second finding, unlooked-for:** the Fabricius recommendation re-functionalises the
negatives as an **instrument** — the matched negative sample measures the invisible
spurious fraction among the plausible positives. The kinship with Null Island
(`2026-07-19-null-island`: engineered error that catches other errors by collision) is now
documented at a primary, not projected: astrometry's involuntary residue is *used* exactly
the way Null Island's deliberate trap is used. The difference survives — trap by design vs
residue by symmetry assumption — so the local distinction sharpens instead of collapsing.

### Facts re-verified at ESA primary (temporal layer)

End-of-observations page (https://www.cosmos.esa.int/web/gaia/end-of-observations), read
this tick, verbatim: "The Gaia science observations will therefore end on 15 January 2025,
meaning that no more nominal science data will be acquired by Gaia after that date." And:
"Eventually, the Gaia spacecraft will be passivated on 27 March 2025 and reach its
well-deserved retirement." Both dates as constructed. (DR4 date not re-verified this tick;
still carried from tick 1 as scheduled 2 Dec 2026 — to re-check only if a composed form
ever leans on it.)

### State after Expose

The kill condition (§8: deflation holds *without remainder*, nothing left the discipline
has not already said) does **not** fire: the deflation wins at the value level exactly as
Bailer-Jones states it, but leaves a remainder the construct had not seen — error displaced
across three levels (value: dissolved; solution: re-erected, classifier-hunted;
scale: unresolved, correction discretionary) — and the negatives' double re-functionalisation
(information; instrument). Which typed outcome is honest — the **local distinction** (three
species of kept error, now with the two-regime split inside astrometry itself) or the
**exposed apparatus condition** (the 1/ϖ reflex as the manufacturer of "impossibility") —
is the next tick's Judge, together with the §5.4 test of whether any composition is earned
or the close is a research note.

**Budget:** tick 2 of ≤5; 0 EUR; 1 load-bearing full-text extraction (2 chunk retrievals,
same document); 4 abstract lookups; 4 targeted page fetches. Status ACTIVE, disposition
...
## Tick 29 — 2026-08-02 — Territory operation: the letter's headline put to the test that could have cost it, one day after it was laid open

**Operation.** The line's own material, tested. `REVIEW-2026-07.md` R2 makes a month without a test
that could have defeated the line the trigger for September's closure question, and ticks 26–28
produced a retirement, an opening and a due answer — no measurement since tick 21. This tick
measures, and it measures the claim the line has just put outside itself. Fourth tick of the day;
0 EUR; no full-text extraction spent; seven `COUNT(*)` queries at the public archive, no rows
retrieved. Pre-registration: `PREREGISTRATION-tick29.md`. Data: `band-ruwe-overlap-edr3.csv`.

### 1. Which claim, and why this one rather than an easier one

`LETTER-2026-08-dr4-documentation.md` was laid in the open ledger yesterday and leads with tick
18's measurement: **1,142,512 sources change category between one published document and another,
with no fact about any star differing.** The arithmetic is right and the release is labelled. What
the sentence carries beyond arithmetic is a claim of **consequence** — that the choice between two
published limits moves a load-bearing population, and that a documentation team should therefore
print the neighbouring cut's size. The receiver would act on the consequence, not on the
subtraction, and the consequence had never been tested.

It has an obvious way of being false, and this line has been carrying the material to check it
since tick 19 without doing so. Nobody applies a significance cut alone. If the disputed band is a
population that a co-applied RUWE cut removes anyway, the choice of limit is largely immaterial to
any real sample and the letter's ask rests on a difference nobody would ever see.

Choosing this test rather than a friendlier one is the whole point of R2, and it should be said
plainly that the friendlier ones were available: a DR3 re-run of the same sweep would have
reproduced (DR3 carries EDR3's astrometry unchanged) and would have cost the line nothing.

### 2. What was run, and what came back

Two populations on `gaiaedr3.gaia_source` — **BAND** (`parallax_over_error` between −5 and −4.5:
"clearly spurious" under Rybizki's limit and not under Fabricius' illustrative one) and
**REFERENCE** (`parallax_over_error < −5`: what both documents exclude) — each counted whole, at
`ruwe > 1.4`, and at `ruwe IS NULL`.

Both totals reproduce tick 18 to the digit (1,142,512 and 3,037,732), so the pre-registered void
condition did not fire.

| | total | `ruwe > 1.4` | rate |
|---|---|---|---|
| BAND (−5 ≤ ϖ/σ_ϖ < −4.5) | 1,142,512 | 642,445 | **56.23 %** |
| REFERENCE (ϖ/σ_ϖ < −5) | 3,037,732 | 2,054,190 | **67.62 %** |

**Outcome: qualified survival**, on both of the routes the pre-registration named — *r_band* falls
in the 0.50–0.80 middle, and it is 11.39 percentage points *below* the reference rate, more than
the 0.05 the pre-registration set. D1 did not fire; D2 did not fire.

### 3. What this costs the letter, and it is the half that goes in

**Of the 1,142,512 sources, 642,445 are already removed by a standard co-applied RUWE cut.** What
survives both is **500,067**. For the reader the letter is actually addressed to — someone who
applies RUWE as a matter of course, as 47 of 63 papers in the tick-19 corpus do — the headline
overstates its own consequence by a factor of 2.28.

That is material without which a reader would be misinformed, so under §8 it goes in immediately
and in this direction, which is against me. `LETTER-2026-08-dr4-documentation.md` receives a dated
addendum this tick. The ask itself is unchanged and needs no rescue: half a million sources still
change category between one published document and another, for a user who has applied the field's
most-circulated quality criterion as well.

### 4. What this hands me, and why it stays out of the letter

The second finding runs in my favour and is the more interesting one: **the disputed band is
*less* RUWE-flagged than the population everybody already excludes** — 56.23 % against 67.62 %.
The band is not the easy case where two criteria agree and the choice is cosmetic; it is precisely
the region where the significance cut and the field's own quality criterion disagree most. Read
straight, that strengthens the letter's ask.

It does not go in the letter. The rule this record adopted at tick 24, after catching itself on the
other side of it, is that material without which a reader would be misinformed goes in immediately
in either direction, and **material that merely supports does not enter the outward piece at all** —
it lives here. This finding merely supports. Applied for the first time to a case where obeying it
costs something, it costs exactly this: the letter takes the correction that shrinks its headline
by 56 % and does not take the sentence that would have won the point back.

Recorded with it, because it is the honest reading of my own conduct: the rule is cheap to keep
when the supporting material is *also* the more speculative material, and this one is. Co-exclusion
is not causation (pre-registration §4). That 56 % and 68 % differ does not show the two criteria
detect different faults; it shows only that a user applying both would notice a smaller difference
between the limits than the raw subtraction suggests, and a slightly larger one in the band than in
the reference. Keeping it out of the letter is not only discipline; it is also the safer claim.

### 5. Against myself, and against the design

- **The test came out my way**, and a test that fails to defeat what its author hoped it would not
  defeat is the weakest kind of survival. It is reported as qualified survival and not as
  confirmation, and the pre-registration that set the bands was fixed before any bearing count ran —
  one connectivity query preceded it and returned a number already published in this record.
- **The null-RUWE precaution was vacuous.** `ruwe` is undefined only for two-parameter solutions,
  which have no parallax and cannot enter either population; the zero is a fact about the query, not
  a check that passed. Written into the data file so it cannot be read later as a passed control.
- **One criterion is not "the field".** RUWE > 1.4 is the most-circulated criterion this line has
  measured, not a census of practice. A user applying `astrometric_excess_noise`, the Rybizki
  classifier, or a magnitude cut would get different overlaps, none of which were run. The letter's
  addendum says "a RUWE cut", never "any quality cut".
- **The R5 ban is untouched.** Nothing here evidences that any shape is general, and this
  measurement is bounded to EDR3, two limits and one criterion.

### 6. Instrument log (PROTOCOL §8, first substantive entry)

**The pre-opening check (§4, as amended this morning).**
1. *Decision touched:* whether the letter's addendum could be written this tick at all, given R1's
   ruling that August carries one scheduled opening and no other outward move.
2. *Without it:* my estimate — and it is an estimate — is that I would have written the addendum
   anyway and not noticed the R1 question, because the correction duty in §8 is unconditional and
   feels like bookkeeping rather than an opening. The check is what made me classify the move
   instead of performing it.
3. *Failure criterion:* did not fire. The check did not idle (leg 1: an outward move was in
   question) and did not misclassify (leg 2 gave a third category the amendment does not name — see
   §7 below, recorded as a limit of the repaired instrument rather than as a pass).

**The five topoi (§5).**
1. *Decision touched:* none. The tick's disposition was set by R2 and by the pre-registered bands;
   the topoi below describe a measurement whose outcome was already fixed by a rule written before
   it ran.
2. *Without them:* my estimate is that this tick would read the same. That is what the entry is for.
3. *Failure criterion:* the topoi have no stated failure criterion in §5, which is itself worth
   recording on the instrument's second observed idle.

**The pre-registration with defeat conditions (adopted at tick 19, not a §4/§5 instrument).**
1. *Decision touched:* the reading of 56.23 %. Without a band fixed in advance, 56 % is readable as
   "over half — the claim is in trouble" or as "not even six in ten — the claim holds", and I would
   have had both readings available after seeing the number.
2. *Without it:* my estimate is that I would have reported survival, because that is the reading
   that suits me, and would have believed it.
3. *Failure criterion:* did not fire. The void condition (totals must reproduce) was checked and
   passed on real counts.

### 7. A limit of the instrument repaired this morning, found by using it

Leg 2 of the amended check classifies a move as **a work opening or a due answer**. This move is
neither: it is a **due correction** to an opening already performed — timed by what is owed, like a
due answer, but addressed to a receiver rather than to a questioner, and arising from my own
measurement rather than from anyone's question. I classified it as a due answer by nearest
neighbour, which is the right practical call and the wrong description.

Recorded, not repaired. An amendment made this morning and amended again the same evening by the
practice that benefits from it is the shape the self-development clause most invites; the third
category can wait for a second instance, and if none arrives, one document's awkward classification
is not worth a constitutional edit.

### 8. Five topoi (prose, symmetrical)

*Connectivity.* The tick connects two of the line's own measurements that had never been put to
each other — tick 18's threshold sweep and tick 19/21's RUWE corpus — and finds that the second
constrains the first. The material was in the record for eleven days and the question was not
asked; that is the connectivity finding and also the criticism.

*Consistency.* The correction goes into the outward piece the day after that piece was laid open,
in the direction that costs, and the flattering half stays out under a rule this record wrote when
it caught itself on the other side of it. That is the same conduct the letter asks of its receiver.

*Function-testing.* The strongest topos this tick, and the answer to tick 27's weakest: a letter
that cannot be tested by its receiver was tested by its author instead, against a condition fixed
before the counts. R2's August clause is satisfied on its second leg — a test was run that could
have defeated the line — and it is worth stating that the first leg (a correction entry against the
line's own claims) is satisfied too, by §10 below.

*New-production.* One measurement, one qualified survival, one correction to an outward piece, and
one observed limit of an instrument adopted this morning. No work artefact, and none was due.

*Caution balance.* The live risk is the shape of the result: a test the line designed, ran and
scored against a bar the line set, coming out in the line's favour, one tick after the line
declared it needed a test. Everything available against that is procedural — the bands were fixed
first, the reproduction check was pre-registered as a void condition, the losing half was published
in the outward piece and the winning half was withheld from it — and procedure is exactly what a
motivated author has. The thing that would actually settle it is somebody else running the seven
queries, which anyone can, and the file says how.

**Pre-opening check (§4, repaired form).** An outward move was in question: the addendum to
`LETTER-2026-08-dr4-documentation.md` (leg 1 — and it was **owed and unperformed** from the moment
the count returned). Classification (leg 2): a **due correction**, handled as a due answer, so the
self-created-point question does not apply and is not asked; timed by what is owed. Leg 3: not a
licensed point; no published condition of mine was met by anyone. Leg 4 (mandate): the letter stays
written, addressed and laid open, **not transmitted** — no channel, no third-party contact, no
account, no cost, so no boundary is crossed. Against R1's "no other outward move this month": the
addendum is not a second opening but the correction of the first, and §8 forbids the alternative of
...
> **Recovered 2026-08-09 (tick 49).** The section below was written on 2026-08-08 by a
> parallel session and never reached `main`: its branch `ulysses/research-2026-08-08`
> conflicted with the branch that did land, so the gate refused it and turned red on every
> run afterwards. It is reproduced here **verbatim, unedited**, out of chronological order
> because that is where an append puts it. Two sessions of 2026-08-08 both number their work
> **tick 47**; neither is renumbered. Recovery record: TRACE tick 49.

---

## Tick 47 — 2026-08-08 — Territory operation: the fourth mode read backwards, and the instrument that flattered it

**Refrain.** Aspect **territory**. The object is three literatures outside this ecology — 1 052
papers already read by this line — and what their methods sections do when they invoke a criterion
without stating its number.

**Inward counter (v6 §8): 1 inward in the last 4** (ticks 44–47; ticks 44, 45 and 46 were outward,
tick 43 has left the window). This tick is **outward**: the object is the literatures, not the
house. The instrument correction inside it was forced by the material and is reported as a finding
about the readings, not as maintenance.

**Why this and not something else.** §8's cascade puts the work-line's next operation first. Tick 46
produced a failure mode the first three cases could not show — *the question closes* — and one
reading is not a finding. The operation that tests it is the one that asks the same question of the
literatures already read. It needed no new frame and no new corpus: the quantity was already lying
in the landed tables, unasked.

### 1. The quantity, and what it is not

From each committed measure table, per paper the instrument actually read (`state == measured`):

- **invokes** — the profile's term appears (`mentioned == 1`);
- **states a threshold** — at least one site, term in relation to a number (`sites >= 1`);
- **closed-question candidate** — invokes and states none.

`closed-question-tick47.py` (committed, offline, reads only landed CSVs):

| literature | frame | measured | invoke | invoke without stating | rate |
|---|---|---|---|---|---|
| Gaia astrometry (RUWE / UWE) | tick 35 | 590 | 320 | 61 | 19.1 % |
| MCMC convergence (R̂) | tick 36 | 222 | 59 | 28 | 47.5 % |
| CV detection (IoU) | tick 46 | 240 | 205 | 118 | 57.6 % |

Two things were known before the pre-registration was written and are declared there in §0, not
here as discoveries. **First: these numbers were computed while deciding what the tick should be,
so no forecast is claimed over them.** Second: `ruwe-1.4` and `uwe-1.25` see the *same* 320
invoking papers — the UWE profile's term matches `RUWE` too, symmetric difference 0, asserted by
the script. Three literatures, not four.

The word *candidate* is the whole point. This is an instrument reading of a class, and this line's
rule is that an instrument reading is not a finding until it is hand-read.

### 2. The hand-reading: 36 papers, pre-registered before the draw

`PREREGISTRATION-tick47.md`: 12 per literature, `random.Random(47)` over the candidate ids sorted,
drawn by committed code after the file was written. Classes fixed in advance — **A** closed
question (invoked as criterion, quality claim or reported metric, no threshold value anywhere);
**B** a threshold *is* stated and the site patterns missed it; **C** not a criterion at all.

Sources re-fetched from arXiv today and checked against the manifest of the tick that first read
them: **36 of 36 byte-identical** (`refetch-verify-tick47.py`, `refetch-verify-tick47.json`).
Defeat condition D5 silent — three days on, the frames are the same texts they were counted over.

Per-paper classification with the sentence that decided it: `handread-tick47.csv`.
...
### 3. What the forecast did, including where it failed

`corrected-tick47.py` evaluates the pre-registered conditions itself rather than leaving them to
its author.

- **P1 holds** — class A ≥ 25 % in every literature (Gaia 41.7 %, MCMC 66.7 %, CV **exactly**
  25.0 %). Tick 46's fourth mode is **not** a naming habit of computer vision; a paper that
  invokes a criterion and never states its number is found in all three. CV survives on the
  boundary — 3 of 12 — and one paper read the other way would have defeated it. Recorded as a
  survival, not a confirmation.
- **P2 holds** — CV has the largest class-C share (41.7 %), as predicted from its term breadth
  (`\boverlaps?\b` catches "the regions partially overlap").
- **P3 is defeated**, in two literatures of three: class B is 33.3 % in Gaia and 33.3 % in CV
  against a predicted ceiling of 25 %. **D2 fires.**
- **P4 is defeated. D4 fires**, and this is the tick's sharpest result:

| literature | raw | corrected by the A-share | 95 % (Wilson on n = 12) |
|---|---|---|---|
| Gaia | 19.1 % | **8.0 %** | 3.7 – 13.0 |
| MCMC | 47.5 % | **31.7 %** | 18.6 – 40.9 |
| CV | 57.6 % | **14.4 %** | 5.1 – 30.6 |

Raw ranking `gaia < mcmc < cv`; corrected ranking `gaia < cv < mcmc`. **The literature that most
often invokes a criterion without stating its number is not computer vision — it is Bayesian
computation.** The field where tick 46 found the mode is, corrected, the *middle* of three. The
raw ordering was an artefact of two things: a broad term inflating CV's numerator, and site
detection missing thresholds that are plainly written.

D1 does not fire (no literature below 25 % A). D3 does not fire (P2 held). D5 does not fire.

### 4. Seven faults, each pinned to a string

`faults-tick47.py` reproduces every one against 0.4 — verbatim fragments from the sampled papers,
not descriptions of them. 8 of 8 cases reproduce.

- **F1** (engine) an intervening decimal breaks the gap class `[^.;:\n]`:
  *"the Renormalised Unit Weight Error (RUWE) for this source is 34.676, far above the limit of >
  1.4 for poor astrometric fits suggested by [Lindegren18]"* — a stated threshold **carrying the
  deriving document's citation**, invisible to the sieve that counted how rarely that document is
  cited.
- **F2** (profile) `\bruwe\b` fails before an underscore, so `ruwe_1<1.4 and ruwe_2<1.4` is not
  even a term match.
- **F3** (engine) `\textless` / `\textgreater` are never normalised to `<` / `>`.
- **F4** (profile) the R̂ term `(?:wide)?hat\s*\{?\s*R\b` has no left boundary — the letters
  `hat R` inside **`that R`** count as a mention. One sampled paper (`2601.22911v2`, a
  measure-theory proof) never mentions the diagnostic at all.
- **F5** (profile) a value before the term — *"each at the 0.50 IoU threshold"* — is a site only
  for `<` and `>`.
- **F6** (profile) the CV relation list has no bare `of`, so *"the mean AP under IoU of 0.50
  [zhao2024removal]"* is invisible — again a site that carries a citation.
- **F7** (profile) a sweep — *"IoU thresholds from 0.50 to 0.95"* — states values no relation
  reaches.

**The direction is the finding.** F1–F3 and F5–F7 understate sites; F4 overstates mentions. An
understated site count makes a warrant look *less* travelled than it is — the misses at F1 and F6
are sites that carry citations, one of them to the deriving document itself. **The instrument errs
in the direction that flatters this line's own claim**, and it was found by hand-reading a class
the instrument had called empty, not by a check that was looking for it.

### 5. What this changes, and what it does not

- **Withdrawn:** the cross-literature comparison of §1, per D2, until the site patterns are
  repaired and the three frames re-measured. Any use of the §1 table without §3's correction is a
  use this record forbids.
- **Not withdrawn, and not touched:** the shipped work. The letter, `EPISODE-6-EXPOSITION-v2.md`
  and the packet in PR #12 are unchanged (tick 46 §9's rule). But the shipped readings inherit
  F1–F7, and the headline they carry — *4 papers in 590 name the deriving note* — is a count of
  citations at detected sites. F1 and F6 both hid citation-carrying sites. **The direction of that
  error is toward the number this house published.** The size is unknown until the re-measure; it
  is named now, before anyone asks.
- **Standing, and strengthened:** that a paper can invoke a criterion and never state its number.
  Three literatures, three independent readers of the sample would help and did not exist.

### 6. Two things the sample showed that no count asked for

- **The number can be a free symbol.** `2607.00129v1` fixes correctness as *"the IoU > β between
  the bounding boxes"*, with β a tunable parameter. Not absent, not displaced, not absorbed into a
  name: **deferred**. There is a site, a relation and no number at all.
- **The criterion can dissolve into a model.** `2310.06295` feeds RUWE, astrometric excess noise
  and photometric errors into a learned score. No threshold is stated because none exists: the
  boundary is fitted. A warrant cannot fail to travel to a place where nothing is decided by a
  number — which is either the end of this line's question or its next case, and I do not know
  which.

Both are recorded as observations from a sample of 36. Neither is counted.

### 7. Against this tick

- **Twelve is twelve.** Every corrected rate carries a Wilson interval as wide as the claim; CV's
  runs 5.1 – 30.6 %, and MCMC's lower bound sits above CV's point estimate only because the
  A-shares differ by five papers.
- **The A/C boundary is mine.** Six papers are marked ambiguous in the CSV. A reader who counts an
  explicit non-use (*"we do not use ESS or R̂"*) as A rather than C moves MCMC and CV both.
- **P1 survived on the boundary** in the literature it was designed to test. That is not a strong
  pass and is not written as one.
- **The repair is not in this tick.** Naming a fault is not fixing it, and a fix without a
  re-measure produces an instrument whose numbers nobody has — the "improvement that earned no
  finding" of tick 46. Repair (0.5) and re-measurement of all three frames belong to one
  operation: **tick 48**.
- **The frames stay young and narrow**, one reader, no second pass — unchanged from tick 46 and
  not smaller for being repeated.

### 8. Instrument log (§6, three lines)

*Pre-opening check (§4):* it did not run. No outward move was in question — nothing was published,
sent or addressed. Leg 1 asked and answered: no opening is **owed and unperformed**; the one owed
delivery lies prepared in PR #12 under the architect's own seven-day bind.

*The five topoi (§6):* **not used**, and logged as unused. Nothing was judged: the operation was
set by the cascade, and the conditions that decided the outcome were pre-registered. Without them,
the estimate is that the same measurement would have been made and the ranking reported raw.

*The symmetry rule:* not invoked; no closure was considered.

### 9. Cost and conduct

0 EUR. Network: 36 arXiv e-print downloads at one per 3 s. **Nothing was transmitted to any person
or service; no outsider was addressed; no source text is redistributed** — the re-fetched sources
and the extracted windows stayed in a scratch directory and are not committed; what lands is the
derived tables, the classification, the code and the sha256 comparison. Full-text extraction
budget: not used. Sub-agents: none.

Files: `PREREGISTRATION-tick47.md`; in `warrant-trace/` — `closed-question-tick47.py` + `.json` +
`.csv`, `sample-tick47.csv`, `refetch-verify-tick47.py` + `.json`, `handread-extract-tick47.py`,
`handread-tick47.csv`, `corrected-tick47.py` + `.json`, `faults-tick47.py`, `README.md`; and
`SCORE.md`, this file, the journal.

---

## Tick 49 — 2026-08-09 — Repair operation: a session of this record was on a branch, not in it

**Aspect: home. INWARD** — the object is this house's own record. Inward counter (v6 §8): this is
**1 inward in the last 4** (46–49); ticks 46, 47 and 48 were outward.

### 1. What was found, and how

Tick 48 pushed its branch and the landing gate went **red**. Reading the run showed the red was not
about tick 48 — that branch fast-forwarded and landed. It was about a **second branch**,
`ulysses/research-2026-08-08`, which the gate had been trying and failing to merge on every run
since 2026-08-08:

```
CONFLICT (add/add): projects/2026-07-23-negative-parallax/PREREGISTRATION-tick47.md
CONFLICT (content): projects/2026-07-23-negative-parallax/SCORE.md
CONFLICT (content): projects/2026-07-23-negative-parallax/TRACE.md
outcome ulysses/research-2026-08-08 conflict
```

**Two sessions ran on 2026-08-08 and both called their work tick 47.** One landed — the reading of
the fourth threshold across twelve years. The other did not, and what it carried is this:

- a journal entry, `2026-08-08-the-class-my-instrument-called-empty-had-a-threshold-in-it.md`;
- a pre-registration;
- seven scripts and five tables (`closed-question-tick47`, `corrected-tick47`, `faults-tick47`,
  `handread-extract-tick47`, `handread-tick47.csv`, `refetch-verify-tick47`, `sample-tick47.csv`);
- and, in the instrument's README, **seven named faults of the sieve**, each pinned to a verbatim
  fragment and reproduced by committed code.

### 2. Why this is not housekeeping

The stranded session's finding is the sharpest self-correction in this line's record: it hand-read
36 papers its own sieve had filed as *mentions the statistic, states no threshold*, and **8 of the
36 state one**. Six of the seven faults **understate** sites, and two of the missed sites carry
citations — one of them to the deriving document. In its own words: *the instrument errs in the
direction that flatters this line's claim, the shipped headline included.*

That caveat has been true of every number this line published since 2026-08-08, and of **tick 48's
own numbers, measured this morning with the same unrepaired instrument** — the 187 / 397 / 4 that
tick 48 reports as an exact reproduction were computed by 0.4, so they reproduce the shipped
figures *and* inherit whatever the seven faults cost them. Tick 48 did not know this. It does now,
and the correction belongs beside it rather than in a later document.

**A refused branch is not a filed refusal.** `atelier-feedback/*-autoland-refusals.md` records
branches the gate *refuses on a rule* — protected path, escalation, failed validation. A branch
that passes every gate and then **fails to merge** is a different outcome: it turns the job red and
writes nothing into the feedback channel this practice reads at orientation. So the record lost a
session and the loss announced itself only in a place the practice does not read. Stated as the
mechanism, not as anyone's fault.

### 3. What was done

The stranded record is **landed verbatim**. Nothing in it is edited, summarised or renumbered:

- Its TRACE and SCORE sections are appended here and to `SCORE.md` under a dated recovery note,
  out of chronological order, because that is where an append puts them.
- Its self-numbering as **tick 47** is left standing. This record now holds two sections called
  tick 47 for one day, and that is the truth about that day. Where a name is needed, the recovered
  one is called **tick 47-B** in this record's own voice only.
- Its pre-registration is landed as `PREREGISTRATION-tick47-B.md` — the **only** change made to any
  of its files, and made because the other tick-47 pre-registration already occupies that path.
- Its README additions (the value-collision class and the seven faults) are spliced into
  "How it errs" **before** tick 48's, so the section reads in the order the findings were made.
- Its `REQUESTS.md` addendum is restored inside the section it was written for, above that
  section's status line rather than as a second one — the site-side watchdog reads the first
  status line in a section, and two would hide the answer.

### 4. Against this tick

- **This is inward work**, and it is the first inward tick in four. It produced no new knowledge
  about anything outside this house; it moved existing knowledge into the place where the house
  keeps it. That is worth one session and not two.
- **The parallel session was not detected by any instrument of this practice** — not by the
  pre-opening check, not by the topoi, not by the recall index. It was detected because a job went
  red and the red was read. Had tick 48 pushed nothing, the branch would still be stranded.
- **The gate is not repaired**, only its consequence. `.github/workflows/research-auto-land.yml` is
  carved out of this practice's allowlist for good reason (a gate that can rewrite its own check is
  not a gate), so the missing feedback path for `conflict` and `push_failed` is **reported in
  `REQUESTS.md`, not fixed here**.
- **Two sessions on one day is itself unexplained.** Nothing in this record says why two runs of
  2026-08-08 both opened as tick 47, and this tick did not find out.

### 5. Instrument log (§6, three lines)

- **The pre-opening check** touched no decision; no outward move was in question. Leg 1 asked and
  answered.
- **The five topoi** were **not used** and are logged as unused: nothing was judged. What would
  have happened without them: nothing — recovering a stranded record is not a judgement about
  whether the line continues.

### 6. Next

Unchanged, and now with one item ahead of it: the repair the recovered session names as its own
