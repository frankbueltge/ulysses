...
# E2b (v2): an edition suffix on a designation token — `A11.1-65`, `ASTM A 53-69`,
# `NFPA 86A-1969`, `ISO 13943:2000`. The token before the separator must contain a digit,
# and may carry one internal space between its letter and number parts (`A 53`, `D 1692`).
RE_DESIG2 = re.compile(
    r"(?<![\w.-])([A-Z][A-Za-z0-9.]*(?:\s\d[A-Za-z0-9.]*|\d[A-Za-z0-9.]*))[-:](\d{2}|\d{4})(?![\d\w-])"
)
# v2: two-digit edition suffixes. This section runs 1941-2019, so a suffix of 30 or more is
# a 20th-century year and anything below is 21st (`ASTM D 56-05` = 2005). Fixed here, and the
# boundary is a judgement recorded in MEASUREMENT.md, not a derived fact.
YY_CENTURY_BREAK = 30
...
def edition_year(text):
    ...
    # v2 precedence, fixed after run 1 voided itself on the blind sample:
    #   (a) an edition suffix carried by the designation itself   -- `A17.1-65`, `ISO 13943:2000`
    #   (b) a parenthesised year                                   -- `API 2000 (1968)`
    #   (c) any bare four-digit year                               -- `..., Apr. 1965`
    # (b) must outrank (c) because a designation *number* can look like a year (`API 2000`).
    suffix = []
    for m in RE_DESIG2.finditer(head):
        g = m.group(2)
        year = int(g) if len(g) == 4 else (1900 + int(g) if int(g) >= YY_CENTURY_BREAK
                                           else 2000 + int(g))
        suffix.append((m.start(), year, "suffix", m.group(0)))
    paren = [(m.start(), int(m.group(1)), "paren", m.group(0))
             for m in re.finditer(r"\((?:[^()]*?)(?<!\d)(19\d{2}|20[0-2]\d)(?!\d)[^()]*?\)", head)]
    bare = [(m.start(), int(m.group(1)), "bare", m.group(0)) for m in RE_YEAR4.finditer(head)]
    for cands in (suffix, paren, bare):
        if cands:
            cands.sort()
